...
jsonrpc_client!(pub struct primeClient {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let cli = App::new("prime")
        ...
        .subcommand(SubCommand::with_name("getpubkey").about("Get node's public key"))
        .subcommand(SubCommand::with_name("newpubkey").about("Generate a random pubkey"))
        .subcommand(
            SubCommand::with_name("mempool").about("Returns the transactions in the mempool"),
        )
        .subcommand(
        ...
        .subcommand(
            SubCommand::with_name("getblock")
                .about("Return the block at number")
                .arg(Arg::with_name("blocknumber").required(true).index(1))
                .about("Block Number"),
        )
        .subcommand(SubCommand::with_name("blockheight").about("Get the current block height"))
        .subcommand(
    ...
    let mut client = primeClient::new(transport_handle);
        ...
        match cli.clone().get_matches_from_safe(args) {
            Ok(matches) => {
                ...
                if let Some(cmd) = matches.subcommand_matches("getbalance") {
                    ...
                    let result = client
                        .getbalance(key::PublicKey::from_str(&pubkey).unwrap())
                        .call()
                        .unwrap();
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
                ...
                if let Some(cmd) = matches.subcommand_matches("send") {
                    ...
                    let result = client
                        .send(key::PublicKey::from_str(&pubkey).unwrap(), amount)
                        .call()
                        .unwrap();
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
